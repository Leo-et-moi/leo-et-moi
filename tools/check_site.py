...
for p in html_files:
    ...
    for m in AUDIO_RE.finditer(txt):
        ...
        if "/" in u:
            ...
            if not target.exists():
                nom = u.split("/")[-1].split("?")[0]
                if nom in AUDIO_ATTENDUS:
                    warn(f"{p.relative_to(ROOT)} : MP3 {nom} pas encore enregistré (liste NOUVEAUX_AUDIOS_A_ENREGISTRER)")
                else:
