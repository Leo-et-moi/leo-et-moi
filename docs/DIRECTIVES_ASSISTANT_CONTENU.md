...
4. Ajouter l'entrée dans `catalog.json` (**avec `ajoute: "AAAA-MM-JJ"`, la date du jour** — alimente la cloche 🔔 des nouveautés et les badges 🆕) : titre, niveau, `ordre` (par dizaines), chemin, `competences` (listening/reading/writing/speaking), liens `lecons`/`exercices` **dans les deux sens**, `progressId` (nouvelle clé = l'ID lui-même), `publie`, et **`ajoute` = la date d'ajout au format `YYYY-MM-DD`** (indispensable : le tableau prof « 📣 Annoncer les nouveautés » ne liste que les items ayant `ajoute` daté de moins de 21 jours ; sans ce champ, la nouveauté n'apparaît pas).
