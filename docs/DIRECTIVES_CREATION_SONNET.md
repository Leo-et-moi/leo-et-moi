...
- **Vocabulaire des boutons (standard du 27/08/2026).** Quand ton brouillon comporte du HTML, tu emploies les classes de `css/site.css` et **rien d'autre** : `.btn-primary` (une seule action principale par écran) · `.btn-secondary` · `.t-play` (audio FR) · `.t-en` (audio EN) · `.opt` avec **`.ok` / `.ng`** pour juste / faux · `.tab` · `.tree-btn`. **Tu ne redéfinis aucune de ces classes dans ton bloc `<style>`** et tu n'inventes pas un nom pour un rôle déjà couvert (`.spk`, `.play-btn`, `.abtn`, `.choice-btn`… : ne plus créer ce genre de nom). Un audit du 27/08/2026 a relevé **105 noms de classes de boutons** différents sur le site, dont une vingtaine pour le seul bouton « lire l'audio en français » — cette dispersion vient en grande partie des blocs `<style>` des livraisons. `.good` / `.bad` sont **dépréciés** : interdits dans du neuf. Rôle réellement nouveau → tu le signales, tu ne l'improvises pas. Spécification : `docs/DIRECTIVES_FABLE.md` §3.
